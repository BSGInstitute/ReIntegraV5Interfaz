...
import {
  ListaResponsableReal,
  PreProcesamientoJson,
...
export class WhatsappComponent implements OnInit {
  ...
  fecha5dias = new Date();
  datosDias: Array<any> = [];
  crearCampania: boolean = true;

  desaparecer: boolean = false;
  crearWhats: boolean = false;
  ...
  ngOnInit(): void {
    ...
    this.fecha5dias.setDate(this.fechaHoy.getDate() + 6);
    this.obtenerUsuario();
  ...
  public ListasWhatsApp: Array<ProcesarListasWhatsAppEnvioAutomatico> = [
    {
      id: 0,
      idConjuntoListaDetalle: 0,
      nombre: '',
      descripcion: '',
      idPlantilla: 0,
      idPersonal: 0,
  ...
  public ListaResponsableReal: Array<ListaResponsableReal> = [
    {
      IdResponsable: 0,
      Total: 0,
    },
  ...
  public PreProcesamientoJson: PreProcesamientoJson = {
    IdCampaniaGeneralDetalle: 0,
    Usuario: '',
    ListaResponsableReal: this.ListaResponsableReal,
  };
  ...
  CambioFecha() {
    ...
    //this.fecha5dias=this.fecha
    this.fecha5dias = new Date(this.fecha.toString());
    this.fecha5dias.setDate(this.fecha5dias.getDate() + 6);
    let i = 0;
  ...
  pruebas(e: any) {
    this.ida = this.autoC;
    this.tablaPrioridades();
    this.autoC = this.listaCampaniasPorWhats.find(
      (option: any) => option.id == this.autoC
    ).nombre;
    this.idFiltroSegmentoTipoContacto = this.listaCampaniasPorWhats.find(
      (option: any) => option.id == this.ida
    ).idFiltroSegmentoTipoContacto;
  }
  private _filter(): string[] {
    return this.listaCampaniasPorWhats.filter((option: any) =>
  ...
  private _filterPlantilla(): string[] {
    return this.listaPlantilla.filter((option: any) =>
      option.nombre.toLowerCase().includes(this.autoPlantillaC.toLowerCase())
    );
  }
  ...
  pruebasResponsable(e: any,index:number) {
    ...
    console.log(this.dataSourceDias)
    console.log(this.dataSourceDias.data[index])
    ...
    this.integraService
      ...
      .subscribe({
        next: (response: HttpResponse<any>) => {
          ...
          //this.idPrioridad = this.listaPrioridades[0].id
    ...
    this.integraService
      ...
      .subscribe({
        next: (response: HttpResponse<any>) => {
          ...
          this.cantidadContactosWhatsapp = js.length;
          if (js.length == 0) {
  ...
  restaDeDias() {
    this.fechamasUno = new Date(this.fechafin.toString());
    this.fechamasUno.setDate(this.fechamasUno.getDate() + 1);
    this.resta = Math.round(
      (this.fechafin.getTime() - this.fecha.getTime()) / (1000 * 60 * 60 * 24)
    );

    if (this.fechafin.getDay() < this.fecha.getDay()) {
      this.resta -= 3;
    }
    ...
    this.listaFechas = getDaysArray(this.fecha, this.fechamasUno);
  ...
  idConfiguracionWhats: any;
  listaConfiguracionWhats: any;
  ...
  crearCampaniaWhats() {
    ...
    if (
      ...
      if (
        ...
        if (
          this.jsonEnvioTotal.tiempoEntreEnvios == -1 ||
          ...
          if (
            ...
            if (
              ...
              if (this.validar == false) {
                ...
                this.integraService
                  .insertar(
                    constApiMarketing.CrearCampaniaWhatsapp,
                    this.jsonEnvioTotal
                  )
                  .subscribe({
                    next: (response: HttpResponse<any>) => {
                      this.listaConfiguracionWhats = response.body;
                      var js = JSON.parse(
                        this.listaConfiguracionWhats.sendingblueRespuesta
                      );
                      this.idConfiguracionWhats = js.Id;
                      // this.fechaInicio = js.fechaDeEnvio
                      // this.fechaFin = js.fechaFinDeEnvio
                      this.loader = false;
                    },
  ...
  AgregarDias() {
    ...
    this.fechaInicio.setDate(this.fechaInicio.getDate() + 2);

    this.fechafin = new Date();
    this.fechafin.setDate(this.fechafin.getDate() + 2);
    ...
    this.rows.forEach((i) => {
      ...
      lw.idConfiguracionDeEnvioParaWhatsApp = this.idConfiguracionWhats;
      lw.dia1 = i.dia1;
      lw.dia2 = i.dia2;
      lw.dia3 = i.dia3;
      lw.dia4 = i.dia4;
    ...
    this.integraService
      .insertar(constApiMarketing.CrearDiasWhatsapp, this.jsonEnvioDiasWhats)
      .subscribe({
        next: (response: HttpResponse<any>) => {
          console.log('Datos respuesta', response.body);
          this.crearWhats = true;
        },
  ...
  creacionWhatsapp() {
    ...
    this.alertaService.mensajeValidacionEnvio().then((result) => {
      if (result.isConfirmed) {
        this.integraService
          .obtener(constApiMarketing.CreacionWhatsapp + '/' + this.ida)
          .subscribe({
            next: (response: HttpResponse<any>) => {
              this.listaCreacionWhats = JSON.parse(response.body);
              // var pgeneral:Array< ProcesarListasWhatsAppEnvioAutomatico> =[]
              this.ListasWhatsApp = [];
              try {
                this.listaCreacionWhats.forEach((p: any) => {
                  // console.log(p);
                  var pg: ProcesarListasWhatsAppEnvioAutomatico = {
                    id: p.Id,
                    nombre: p.Nombre,
                    descripcion: p.descripcion,
                    idConjuntoListaDetalle: p.idConjuntoListaDetalle,
                    idPlantilla: p.idPlantilla,
                    idPersonal: p.idPersonal,
                    idConjuntoLista: p.idConjuntoLista,
                    programaPrincipal: [
                      {
                        id: 0,
                        fechaCreacion: new Date(),
                        ...
                        usuarioModificacion: '',
                        rowVersion: '',
                        idWhatsAppConfiguracionEnvio:
                          p.idWhatsAppConfiguracionEnvio,
                        idPgeneral: p.idPgeneral,
                        idTipoEnvioPrograma: p.idTipoEnvioPrograma,
                      },
                    ],
                    programaSecundario: [
                      {
                  ...
                  this.ListasWhatsApp.push(pg);
                  if (this.ListasWhatsApp.length == 3000) {
                    setTimeout(() => {
                      this.ProcesarListasWhatsAppEnvioAutomaticoPost();
                      this.ListasWhatsApp = [];
                    }, 3000);
                  } else {
                  }
  ...
  ProcesarListasWhatsAppEnvioAutomaticoPost() {
    this.integraService
      .insertar(
        constApiMarketing.ProcesarListasWhatsappEnvioAutomatico,
        this.ListasWhatsApp
      )
      .subscribe({
        next: (response: HttpResponse<any>) => {
  ...
  PreProcesamiento() {
    (this.PreProcesamientoJson.IdCampaniaGeneralDetalle =
      this.jsonEnvioTotal.idCampaniaGeneralDetalle),
      (this.PreProcesamientoJson.Usuario = this.usuarioLog.usuario),
      (this.ListaResponsableReal = []),
      this.jsonEnvioDiasWhats.forEach((p: any) => {
        var pg: ListaResponsableReal = {
          IdResponsable: p.idPersonal,
          Total: p.total,
        };
        this.ListaResponsableReal.push(pg);
      });
    this.PreProcesamientoJson.ListaResponsableReal = this.ListaResponsableReal;
    this.integraService
      .insertar(
        constApiMarketing.FinalizarPreProcesamientoWhatsApp,
        this.PreProcesamientoJson
      )
      .subscribe({
        next: (response: HttpResponse<any>) => {
        },
