...
export class WhatsappModalComponent implements OnInit {
        ...
        next: (response: HttpResponse<any>) => {
          ...
          if (this.listaAreaPorCampania.length > 0) {
            ...
            if (
              ...
              this.selectArea = 'programaGeneral';
              this.listaAreaPorCampania[0].idCampaniaGeneralDetallePrograma.forEach((e:any) => {
              ...
              this.jsonEnvio.subArea = this.listaAreaPorCampania[0].idSubAreaCapacitacion;
              this.jsonEnvio.area = this.listaAreaPorCampania[0].idAreaCapacitacion;
            } else if (
  ...
  obtenerPais() {
    this.loading = true;
    this.integraService.obtener(constApiGlobal.PaisObtenerCombo).subscribe({
      next: (response: HttpResponse<any>) => {
        console.log(response.body);
        this.loading = false;
  ...
  pruebas(e: any) {
    ...
    console.log("🚀 ~ file: configuracion-prioridades.component.ts:676 ~ ConfiguracionPrioridadesComponent ~ pruebas ~ listita", listita)
