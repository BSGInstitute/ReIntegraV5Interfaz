...
export class SesionesComponent implements OnInit {
  ...
  Lengt:any
  displayedColumns: string[] = ['estadoSesion', 'operador','cantidad', 'unidadDeTiempor', 'acciones'];
  ...
  estadoSesion: Array<EstadoPago> = [
    { Id : 1, Nombre : "Sesion proxima dentro de" },
    { Id : 2, Nombre : "Sesion pasada hace" },
 ];
    ...
    this.integraService.obtener(constApiMarketing.ObtenerFrecuenciaTiempo).subscribe({
      next: (response: HttpResponse<any>) => {
        ...
        if (this.datosActualizar != undefined) {
          this.datosActualizar.listaSesion.forEach((e: any) => {
            this.estadoSesion.forEach((f: any) => {
              if (e.valor == f.Id) {
