...
import {
  ...
  ReporteTasaCambio,
  ReporteV2Integra,
} from '../../models/interfaces/ireporte-cambio-fase';
import { UserService } from '@shared/services/user.service';

@Component({
  selector: 'app-reporte-cambio-fase',
  templateUrl: './reporte-cambio-fase.component.html',
  styleUrls: ['./reporte-cambio-fase.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class ReporteCambioFaseComponent implements OnInit {
  constructor(
  ...
  generarReporteCobertura() {
    ...
    this.iniciarLoading();
    this.generarReporteV2TasaContactoAsyncReplica(filtro);
    this.generarReporteV2ControlBicYEAcumuladoAsyncReplica(filtro); 
    this.generarReporteV2AsyncReplica(filtro);
    // this.generarReporteCalidadProcesamientoReplica(filtro);
    this.generarReporteCalidadProcesamiento(filtro);
    if(dataForm.fechaFin < new Date('2023-10-12T00:00:00')){
      this.flagGridConteoDatosFase = true;
      this.obtenerReporteConteoDatosFase(filtro); //Replica
    }else{
      this.flagGridConteoDatosFase = false;
      this.obtenerReporteConteoDatosFaseAlterno(filtro); //Replica
    }

    this.generarReporteV2IntegraAsync(filtro);
  }
  ...
  generarReporteV2ControlBicYEAcumuladoAsyncReplica(filtro:any){
    this.integraReplicaService
    .postJsonResponse(
      constApiComercial.ReporteCambioDeFaseGenerarReporteV2ControlBicYEAcumuladoAsync,
  ...
  generarReporteV2TasaContactoAsyncReplica(filtro:any){
    this.integraReplicaService
    .postJsonResponse(
      constApiComercial.ReporteCambioDeFaseGenerarReporteV2TasaContactoAsync,
      JSON.stringify(filtro)
    )
    .subscribe({
      next: (resp: HttpResponse<IReporteTasaContactoAmbos>) => {
        ...
        this.generarReporteLlamadaActividad(filtro);
      },
      error: (error) => {
  ...
  generarReporteV2AsyncReplica(filtro: any) {
    this.integraReplicaService
    .postJsonResponse(
      constApiComercial.ReporteCambioDeFaseGenerarReporteV2Async,
      JSON.stringify(filtro)
    )
    .subscribe({
  ...
  generarReporteV2IntegraAsync(filtro: any) {
    this.integraService
        .postJsonResponse(
          constApiComercial.ReporteCambioDeFaseGenerarReporteV2IntegraAsync,
          JSON.stringify(filtro)
        )
        .subscribe({
          next: (resp: HttpResponse<ReporteV2Integra>) => {
            ...
            if (resp.body != null) {
              ...
              this.setDataGridActividadVencida(
                resp.body.actividadVencidaporTab
              );
  ...
  generarReporteCalidadProcesamiento(filtro: any) {
    this.integraReplicaService
        ...
        .subscribe({
          next: (
            resp: HttpResponse<IReporteCalidadProcesamientoCambioFase>
          ) => {
            this.btnBuscarDisabled = false;
            if (resp.body != null) {
              if (resp.body.reporteCalidadProcesamiento.length != 0) {
                this.setDataGridCalidadAlterno(resp.body.reporteCalidadProcesamiento);
              }
              else{
  ...
  //Nuevo
  obtenerReporteConteoDatosFase(filtro: any) {
    this.integraReplicaService
        .postJsonResponse(
  ...
  obtenerReporteConteoDatosFaseAlterno(filtro: any) {
    this.integraService
        .postJsonResponse(
          constApiComercial.ReporteCambioDeFaseObtenerReporteConteoDatosFaseAlterno,
          JSON.stringify(filtro)
        )
        .subscribe({
          next: (
            resp: HttpResponse<ConteoDatosFasePais[]>
          ) => {
            this.btnBuscarDisabled = false;
            if (resp.body != null) {
  ...
  generarReporteCalidadProcesamientoReplica(filtro: any) {
    this.integraReplicaService
        .postJsonResponse(
          constApiComercial.ReporteCambioDeFaseGenerarReporteCalidadProcesamiento,
          JSON.stringify(filtro)
  ...
  setDataGridCalidadAlterno(reporteCalidadProcesamiento: ICalidadProcesamientoAlterno[]) {
    let asesorArreglo: ICalidadProcesamientoAlterno[] =
      reporteCalidadProcesamiento.filter(
        (asesor) => asesor.datosAsesor != null
      );
    asesorArreglo = asesorArreglo.sort((a, b) => a.datosAsesor.localeCompare(b.datosAsesor));
  ...
  setDataConteoDatosFase(
    ...
    let fases: string[] = ['BNC', 'BNC-1', 'IT', 'IP', 'PF', 'IC', 'RN', 'Total'];
    fases.forEach((fase) => {
  ...
  setDataConteoDatosFaseAlterno(
    conteoDatosFase: ConteoDatosFasePais[]) {
    ...
    res.forEach(x => {
      x.inicioTotal = x.inicioPeru + x.inicioColombia + x.inicioMexico + x.inicioChile + x.inicioOtros;
      x.momentoTotal = x.momentoPeru + x.momentoColombia + x.momentoMexico + x.momentoChile + x.momentoOtros;
    });
  ...
  setDataGridTasaConversion(dataTasaConversion: {
    ...
    let divisionPromedios = {
      ...
      chileMes: this.divideZero(
        chileMensual.pP_IM_USD,
        chileMensual.pP_OC_USD
      ),
  ...
  setDataGridControlActividades(controlCambiodeFase: IControlCambioFase[]) {
    ...
    this.gridControlCambiodeFase.data = [
      ...
      {
        descripcion: 'Número de actividades reprogramadas manuales',
      ...
      {
        descripcion: 'Número de actividades ejecutadas sin llamada',
  ...
  setDataReporteGeneral(
    ...
    let respuestasMetas: any = [];
    respuesta.forEach((element: any) => {
      let a = myMap.get(element.faseOrigen);
      respuestasMetas.push({
  ...
  generarReporteLlamadaActividad(filtro: any){
    this.integraReplicaService.postJsonResponse(constApiComercial.ReporteCambioDeFaseGenerarReporteActividadEjecutadaLlamadaObservada, JSON.stringify(filtro)).subscribe({
    ...
    //Cambio a produccion
    this.integraService.postJsonResponse(constApiComercial.ReporteCambioDeFaseGenerarAcumuladoTiempoContactoEfectivo, JSON.stringify(filtro)).subscribe({
    ...
    this.integraReplicaService.postJsonResponse(constApiComercial.ReporteCambioDeFaseGenerarAcumuladoLlamadasReprogramadasManualmente, JSON.stringify(filtro)).subscribe({
      next: (resp: HttpResponse<ILlamadaObservada[]>) => {
    ...
    // Cambio a produccion
    this.integraService.postJsonResponse(constApiComercial.ReporteCambioDeFaseGenerarActividadEjecutadaFaseActual, JSON.stringify(filtro)).subscribe({
  ...
  cargarGridLlamadaObservadas(actividadEjecutadaLlamadaObservada: ILlamadaObservada[]){
    let campos = [
    ...
    let data: any =  []
    campos.forEach(e => {
      let bnc = actividadEjecutadaLlamadaObservada.find(x => x.faseOportunidad == 'BNC' && x.caso == e);
      let bnc_1 = actividadEjecutadaLlamadaObservada.find(x => x.faseOportunidad == 'BNC-1' && x.caso == e);
      let it = actividadEjecutadaLlamadaObservada.find(x => x.faseOportunidad == 'IT' && x.caso == e);
      let ip = actividadEjecutadaLlamadaObservada.find(x => x.faseOportunidad == 'IP' && x.caso == e);
      let ic= actividadEjecutadaLlamadaObservada.find(x => x.faseOportunidad == 'IC' && x.caso == e);
      let pf = actividadEjecutadaLlamadaObservada.find(x => x.faseOportunidad == 'PF' && x.caso == e);
      let is = actividadEjecutadaLlamadaObservada.find(x => x.faseOportunidad == 'IS' && x.caso == e);
      let m = actividadEjecutadaLlamadaObservada.find(x => x.faseOportunidad == 'M' && x.caso == e);
  ...
  cargarGridActividadesEjecutadasFaseActual(actividadEjecutadaFaseActual: IActividadEjecutadaFaseActual[]){
    let campos = [
      '0',
      '1',
      '2',
    ...
    let data: any =  []
    campos.forEach(e => {
      let bnc = actividadEjecutadaFaseActual.find(x => x.faseOportunidad == 'BNC' && x.caso == e);
      let bnc_1 = actividadEjecutadaFaseActual.find(x => x.faseOportunidad == 'BNC-1' && x.caso == e);
      let it = actividadEjecutadaFaseActual.find(x => x.faseOportunidad == 'IT' && x.caso == e);
      let ip = actividadEjecutadaFaseActual.find(x => x.faseOportunidad == 'IP' && x.caso == e);
      let ic= actividadEjecutadaFaseActual.find(x => x.faseOportunidad == 'IC' && x.caso == e);
      let pf = actividadEjecutadaFaseActual.find(x => x.faseOportunidad == 'PF' && x.caso == e);
      let is = actividadEjecutadaFaseActual.find(x => x.faseOportunidad == 'IS' && x.caso == e);
      let m = actividadEjecutadaFaseActual.find(x => x.faseOportunidad == 'M' && x.caso == e);
      let cantidadIs = is != null ? is.cantidad : 0
      let cantidadM = m != null ? m.cantidad : 0
      ...
      data.push({
        caso: e,
        BNC: bnc != null ? `${bnc.cantidad} (${bnc.promedioLlamada.toFixed(1) }min)` : '0(0)' ,
        BNC_1: bnc_1 != null ? `${bnc_1.cantidad} (${bnc_1.promedioLlamada.toFixed(1) }min)` : '0(0)' ,
        IT: it != null ? `${it.cantidad} (${it.promedioLlamada.toFixed(1)} min)` : '0(0)' ,
        IP: ip != null ? `${ip.cantidad} (${ip.promedioLlamada.toFixed(1)} min)` : '0(0)' ,
        IC: ic != null ? `${ic.cantidad} (${ic.promedioLlamada.toFixed(1)} min)` : '0(0)' ,
        PF: pf != null ? `${pf.cantidad} (${pf.promedioLlamada.toFixed(1)} min)` : '0(0)' ,
  ...
  cargarGridAcumuladoLlamadas(acumuladoLlamadasReprogramadasManualmente: ILlamadaObservada[]){
    let campos = [
      '1',
      '2',
      '3',
    ...
    let data: any =  []
    campos.forEach(e => {
      let bnc = acumuladoLlamadasReprogramadasManualmente.find(x => x.faseOportunidad == 'BNC' && x.caso == e);
      let bnc_1 = acumuladoLlamadasReprogramadasManualmente.find(x => x.faseOportunidad == 'BNC-1' && x.caso == e);
      let it = acumuladoLlamadasReprogramadasManualmente.find(x => x.faseOportunidad == 'IT' && x.caso == e);
      let ip = acumuladoLlamadasReprogramadasManualmente.find(x => x.faseOportunidad == 'IP' && x.caso == e);
      let ic= acumuladoLlamadasReprogramadasManualmente.find(x => x.faseOportunidad == 'IC' && x.caso == e);
      let pf = acumuladoLlamadasReprogramadasManualmente.find(x => x.faseOportunidad == 'PF' && x.caso == e);
      let is = acumuladoLlamadasReprogramadasManualmente.find(x => x.faseOportunidad == 'IS' && x.caso == e);
      let m = acumuladoLlamadasReprogramadasManualmente.find(x => x.faseOportunidad == 'M' && x.caso == e);
      let cantidadIs = is != null ? is.cantidad : 0
      let cantidadM = m != null ? m.cantidad : 0
      data.push({
        caso: e,
  ...
  cargarGridAcumuladoTiempoContacto(acumuladoTiempoContactoEfectivo: IAcumuladoTiempoContactoEfectivo[]){
    let campos = [
      'Menos de 2',
      '2.0-5.0',
      '5.1-8.0',
    ...
    let data: any =  []
    campos.forEach(e => {
      let bnc = acumuladoTiempoContactoEfectivo.find(x => x.faseOportunidad == 'BNC' && x.tiempoContacto == e);
      let bnc_1 = acumuladoTiempoContactoEfectivo.find(x => x.faseOportunidad == 'BNC-1' && x.tiempoContacto == e);
      let it = acumuladoTiempoContactoEfectivo.find(x => x.faseOportunidad == 'IT' && x.tiempoContacto == e);
      let ip = acumuladoTiempoContactoEfectivo.find(x => x.faseOportunidad == 'IP' && x.tiempoContacto == e);
      let ic= acumuladoTiempoContactoEfectivo.find(x => x.faseOportunidad == 'IC' && x.tiempoContacto == e);
      let pf = acumuladoTiempoContactoEfectivo.find(x => x.faseOportunidad == 'PF' && x.tiempoContacto == e);
      let is = acumuladoTiempoContactoEfectivo.find(x => x.faseOportunidad == 'IS' && x.tiempoContacto == e);
      let m = acumuladoTiempoContactoEfectivo.find(x => x.faseOportunidad == 'M' && x.tiempoContacto == e);
      let cantidadIs = is != null ? is.total : 0
      let cantidadM = m != null ? m.total : 0

      let rn2a = acumuladoTiempoContactoEfectivo.find(x => x.faseOportunidad == 'RN2-A' && x.tiempoContacto == e);
      let rn2b = acumuladoTiempoContactoEfectivo.find(x => x.faseOportunidad == 'RN2-B' && x.tiempoContacto == e);
      let rn2c = acumuladoTiempoContactoEfectivo.find(x => x.faseOportunidad == 'RN2-C' && x.tiempoContacto == e);
      let rn3 = acumuladoTiempoContactoEfectivo.find(x => x.faseOportunidad == 'RN3' && x.tiempoContacto == e);
      let rn1 = acumuladoTiempoContactoEfectivo.find(x => x.faseOportunidad == 'RN1' && x.tiempoContacto == e);
      let eliminar = acumuladoTiempoContactoEfectivo.find(x => x.faseOportunidad == 'E' && x.tiempoContacto == e);
      let bic = acumuladoTiempoContactoEfectivo.find(x => x.faseOportunidad == 'BIC' && x.tiempoContacto == e);
      let nsi = acumuladoTiempoContactoEfectivo.find(x => x.faseOportunidad == 'NS' && x.tiempoContacto == e);
      let otros = acumuladoTiempoContactoEfectivo.find(x => x.faseOportunidad == 'OTROS' && x.tiempoContacto == e);
