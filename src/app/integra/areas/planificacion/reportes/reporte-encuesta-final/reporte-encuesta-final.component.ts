 ...
 * @description Componente del Reporte Encuesta Final
 * @author Jonathan Raúl Caipo Huacasi
 * @version 1.0.0
 * @history
   18/05/2023 Implementacion del Reporte Encuesta Final
   18/05/2023 Creacion de Grilla
 */

@Component({
  selector: 'app-reporte-encuesta-final',
  templateUrl: './reporte-encuesta-final.component.html',
  styleUrls: ['./reporte-encuesta-final.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class ReporteEncuestaFinalComponent implements OnInit {
  constructor(
  ...
  gridReporteEncuestaFinal: KendoGrid = new KendoGrid();
  ...
  generarReporte(){
    if (this.formFiltro.valid) {
      this.gridReporteEncuestaFinal.loading = true;
      ...
      this.integraService
        .postJsonResponse(
          constApiPlanificacion.ReporteEncuestasGenerarReporteEncuestaFinal,
          JSON.stringify(filtro)
        )
        .subscribe({
          next: (resp: HttpResponse<ReporteEncuesta[]>) => {
            this.gridReporteEncuestaFinal.data = resp.body;
            this.gridReporteEncuestaFinal.loading = false;
  ...
  calcularColumnaPreguntas(data: ReporteEncuesta[]){
    ...
    data[0].preguntas.forEach(p => {
      if([5,3].includes(p.idTipoPregunta)){
  ...
  crearDataParaExcel(){
    this.data = [];
    this.gridReporteEncuestaFinal.data.forEach((x: ReporteEncuesta) => {
      let obj: any = {}
      obj.alumno = x.alumno;
