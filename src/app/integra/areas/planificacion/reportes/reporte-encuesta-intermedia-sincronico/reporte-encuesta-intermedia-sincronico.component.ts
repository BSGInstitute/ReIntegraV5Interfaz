 ...
 * @description Componente del Reporte Encuesta Intermedia de cursos Sincrónicos
 * @author Max Mantilla Rodriguez - Jeremy Pacheco
 * @version 1.0.0
 * @history
   12/11/2024 Implementacion del Reporte Encuesta Intermedia
   22/11/2024 Creacion de Grilla
 */

@Component({
  selector: 'app-reporte-encuesta-intermedia-sincronico',
  templateUrl: './reporte-encuesta-intermedia-sincronico.component.html',
  styleUrls: ['./reporte-encuesta-intermedia-sincronico.component.scss'],
})
export class ReporteEncuestaIntermediaSincronicoComponent implements OnInit {
  constructor(
  ...
  gridReporteEncuestaIntermediaSincronico: KendoGrid = new KendoGrid();
  ...
  generarReporte() {
    if (this.formFiltro.valid) {
      this.gridReporteEncuestaIntermediaSincronico.loading = true;
      ...
      this.integraService
        .postJsonResponse(
          constApiPlanificacion.ReporteEncuestasGenerarReporteEncuestaIntermediaSincronico,
          JSON.stringify(filtro)
        )
        .subscribe({
          next: (resp: any) => {
            console.log('ResultadoReporte',resp);
            this.gridReporteEncuestaIntermediaSincronico.data = resp.body;
            this.calcularColumnaPreguntas(this.gridReporteEncuestaIntermediaSincronico.data);
            this.calcularColumnaExcelDesdeGrilla();
            this.gridReporteEncuestaIntermediaSincronico.loading = false;
  ...
  crearDataParaExcel() {
    ...
    this.gridReporteEncuestaIntermediaSincronico.data.forEach((x: any) => {
      const obj: any = {};
