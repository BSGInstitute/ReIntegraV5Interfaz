...
export class RevisarAprobarMaterialComponent implements OnInit {
  ...
  aprobarMaterial(dataItem: MaterialPEspecifico) {
    this.gridRevisarAprobarMaterial.loading = true;
    this.integraService
      .putJsonResponse(
        `${constApiPlanificacion.MaterialPespecificoAprobarMaterialVersion}/${dataItem.idMaterialPEspecificoDetalle}`, null
      )
      .subscribe({
        next: (response: HttpResponse<boolean>) => {
          ...
          this.gridRevisarAprobarMaterial.loading = false;
          this.generarReporte();
          this.alertaService.notificationSuccessBotom("Material Aprobado Correctamente.");
        },
        error: (error) => {
          this.gridRevisarAprobarMaterial.loading = false;
          let mensaje = this.alertaService.getMessageErrorService(error);
  ...
  desaprobarMaterial(dataItem: MaterialPEspecifico) {
    console.log("dataItem", dataItem)
    this.gridRevisarAprobarMaterial.loading = true;
    this.integraService
      .getJsonResponse(
        `${constApiPlanificacion.MaterialPespecificoDesaprobarMaterialVersion}/${dataItem.idMaterialPEspecificoDetalle}`
      )
      .subscribe({
        next: (response: HttpResponse<boolean>) => {
          console.log(response.body);
          Swal.fire(
            '¡Desaprobado!',
            'Material Desaprobado Correctamente.',
            'success'
          );
          this.gridRevisarAprobarMaterial.loading = false;
          this.generarReporte();
          this.alertaService.notificationSuccessBotom("Material Desaprobado Correctamente.");
        },
        error: (error) => {
          this.gridRevisarAprobarMaterial.loading = false;
          let mensaje = this.alertaService.getMessageErrorService(error);
