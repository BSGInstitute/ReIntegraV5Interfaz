...
import { IFiltroActualizarSesiones, IFiltroConfiguraGrabacionesOnline, IFiltroIniciarProcesoResumenGrabaciones, IFiltroInsertarConfiguracionResumenGrabacionOnline, IFiltroModificarDisponibilidadProgramaDefecto, IFiltroObtenerSesiones } from '@planificacion/models/interfaces/configura-grabaciones-online';
...
export class ConfigurarGrabacionesOnlineComponent implements OnInit {
  ...
  obtenerAreaCombo(): void {
    this.integraService.obtenerTodo(constApiPlanificacion.AreaCapacitacionObtenerCombo)
    .subscribe({
  ...
  obtenerProgramaGeneralCombo(): void {
    this.integraService.obtenerTodo(constApiPlanificacion.ProgramaGeneralObtenerProgramasGenerales)
    .subscribe({
      next: (response: HttpResponse<any>) => {
  ...
  obtenerSubArea(idArea: number[]) {
    ...
    if (idArea.length > 0) {
      this.integraService.postJsonResponse(constApiPlanificacion.ObtenerSubAreaPorIdDeArea, JSON.stringify(idArea)).subscribe({
  ...
  obtenerProgramaEspecifico(idProgramaGeneral: number[]) {
    ...
    if (idProgramaGeneral.length > 0) {
      this.integraService.postJsonResponse(constApiPlanificacion.PEspecificoObtenerCombosPEpecificoPorProgramaGeneral, JSON.stringify(idProgramaGeneral)).subscribe({
        next: (resp: HttpResponse<IComboBase1[]>) => {
  ...
  obtenerPartnerCombo(): void {
    this.integraService.obtenerTodo(constApiPlanificacion.PartnerPwObtenerCombo)
    .subscribe({
      next: (response: HttpResponse<any>) => {
  ...
  obtenerDiasDisponibles(): void {
    this.integraService.obtenerTodo(constApiPlanificacion.ObtenerDisponibilidadPrograma)
    .subscribe({
      next: (response: HttpResponse<any>) => {
  ...
  eliminaItemsProgramasEspecificosSeleccionados() {
    if (this.selectedProgramaEspecifico) {
      this.selectedProgramaEspecifico = this.selectedProgramaEspecifico.filter(programaEspecificoId => {
        const programasEspecificos = this.programaEspecificoFiltrado.find(p => p.id === programaEspecificoId);
        return programasEspecificos && this.programaGeneralFiltrado.includes(programasEspecificos.id);
      });
    }
  }
  ...
  buscarVisitaProgramasOnline() {
    ...
    this.integraService.postJsonResponse(constApiPlanificacion.GenerarVistaProgramasOnline, filtro).subscribe({
      next: (resp: any) => {
        if (resp != null) {
  ...
  abrirConfiguracionGeneral(modalSesion: any, dataItem?: any) {
    ...
    this.integraService.postJsonResponse(constApiPlanificacion.ObtenerSesiones, filtro).subscribe({
      next: (resp: any) => {
        if (resp != null) {
          this.gridSesiones = resp.body;
          this.gridSesiones.forEach((item: any) => {
            item.fechaInicio = item.fechaInicio ? new Date(item.fechaInicio) : null;
            item.fechaFin = item.fechaFin ? new Date(item.fechaFin) : null;
            item.habilitado = item.habilitado == "true" ? "Habilitado" : "Deshabilitado";
          });
    ...
    this.modalRefSesiones = this.modalService.open(modalSesion, { size: 'xl', backdrop: 'static' });
  }

  obtenerResumenGrabacionOnlineTiposHabilitados() {
    this.tipoResumenHabilitado = [];
    this.integraService.getJsonResponse(constApiPlanificacion.ObtenerResumenGrabacionOnline).subscribe({
      next: (resp: any) => {
        this.tipoResumenHabilitado = (resp.body as any[]).map(item => item.id);
      },
    });
  }
  ...
  abrirDetalleResumenes(modalDetalleResumen: any, dataItem: any) {
    ...
    this.integraService.postJsonResponse(constApiPlanificacion.ObtenerDetalleResumenGrabacionSesion, filtro).subscribe({
      next: (resp: any) => {
        if (resp != null) {
    ...
    this.modalRefDetalleResumen = this.modalService.open(modalDetalleResumen, { size: 'xl', backdrop: 'static' });
  }

  descargarTextoOriginal(id: number): void {
    this.integraService.postJsonResponse(constApiPlanificacion.ObtenerTextoTranscripcionPorId, id)
  ...
  descargarResumenAudio(id: number): void {
    this.integraService.postJsonResponse(constApiPlanificacion.ObtenerTextoGuionAudioPorId, id)
    .subscribe((response: any) => {
      const texto = response?.body?.texto;
      if (texto && texto.trim()) {
        this.descargarArchivoTxt(texto, `TranscripcionResumenAudio_${id}.txt`);
      } else {
        console.warn('El texto recibido está vacío o no es válido.');
      }
    });
  ...
  abrirConfiguracionResumen(modalConfiguracionResumen: any, dataItem: any) {
    ...
    this.iniciaChecks();
    this.modalRefConfiguracionResumen = this.modalService.open(modalConfiguracionResumen, { size: 'lg', backdrop: 'static' });
  }

  iniciaChecks() {
    this.integraService.postJsonResponse(constApiPlanificacion.ObtenerConfiguracionResumenGrabacionOnlinePorSesion, this.dataItemSeleccionado.idPEspecificoSesion)
    .subscribe({
      next: (response: HttpResponse<any[]>) => {
  ...
  async iniciaProcesoResumen(dataItem: any) {
    ...
    if (!dataItem.video || dataItem.video.trim() === '') {
      Swal.fire('Error!', 'Para iniciar el proceso de resumen, debe tener asignado un Id de video.', 'warning');
      return;
    }
    //Obtiene configuración actual de interfaz de configuración para resúmenes desde Integra
    const configuracionItemResumen = await this.obtenerConfiguracionResumenGrabacionPorSesion(dataItem);
    //Valida que al menos un tipo de resumen se encuentre configurado
    const tipoResumenGrabacionOnline = configuracionItemResumen.map((valor, index) => valor ? index + 1 : null).filter(numero => numero !== null);
    if (!tipoResumenGrabacionOnline || tipoResumenGrabacionOnline.length === 0) {
      Swal.fire('Error!', 'No existen tipos de resumen configurados para esta sesión', 'warning');
      return;
    }
    ...
    const datos: IFiltroIniciarProcesoResumenGrabaciones = {
      ...
      tipoResumenGrabacionOnline: tipoResumenGrabacionOnline,
      sesion: dataItem.nombreSesion,
      urlVideo: dataItem.video,
      usuario: this.userService.userName
    };
    this.integraService.postJsonResponse(constApiPlanificacion.GenerarResumenGrabaciones, datos)
    .subscribe({
      next: () => {
  ...
  async obtenerConfiguracionResumenGrabacionPorSesion(dataItem: any): Promise<boolean[]> {
    return new Promise((resolve, reject) => {
      this.integraService.postJsonResponse(constApiPlanificacion.ObtenerConfiguracionResumenGrabacionOnlinePorSesion, dataItem.idPEspecificoSesion)
        .subscribe({
          next: (response: HttpResponse<any[]>) => {
            if (response.body && response.body.length > 0) {
  ...
  guardarCambiosSesiones(): void {
    ...
    const filtro: IFiltroActualizarSesiones[] = this.gridSesiones.map((item: any) => ({
    ...
    this.integraService.postJsonResponse(constApiPlanificacion.ActualizarSesiones, { Data: filtro }).subscribe({
      next: (resp: any) => {
      ...
      error: (err) => {
        this.gridSesiones.loading = false;
        Swal.fire('Error!', 'Ocurrió un problema al guardar los cambios.', 'warning');
        this.isGuardarDisabled = false;
      },
    });
  }
  ...
  guardarCambiosConfiguracionResumenes(): void {
    ...
    this.integraService.postJsonResponse(constApiPlanificacion.InsertarEliminarConfiguracionResumenGrabacionOnline, datos).subscribe({
      next: (resp: any) => {
  ...
  ModificarDisponibilidadPrograma() {
    ...
    this.integraService.postJsonResponse(constApiPlanificacion.ModificarDisponibilidadProgramaDefecto, filtro).subscribe({
      next: (resp: any) => {
        this.loading = false;
      },
    });
  ...
  ngAfterViewChecked(): void {
    ...
    if (this.editingField === 'fechaInicio' || this.editingField === 'fechaFin') {
      const datepickerWrapper = document.querySelector('.editable-cell-container kendo-datepicker') as HTMLElement | null;
      if (datepickerWrapper) {
        inputElement = datepickerWrapper.querySelector('input');
      }
    } else {
      inputElement = document.querySelector('.editable-input') as HTMLInputElement | null;
    }
    if (inputElement) {
      inputElement.focus();
    }
  ...
  onVideoBlur(dataItem: any) {
    ...
    const diasDisponibles = parseInt(this.diasDisponibles.toString().trim(), 10);
