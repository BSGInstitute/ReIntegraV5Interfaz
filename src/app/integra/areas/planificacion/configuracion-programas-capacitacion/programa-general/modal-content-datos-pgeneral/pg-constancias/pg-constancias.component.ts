 ...
 * @description Componente de Constancias de programas generales
 ...
 **/
@Component({
  selector: 'app-pg-constancias',
  templateUrl: './pg-constancias.component.html',
  styleUrls: ['./pg-constancias.component.scss'],
})
export class PgConstanciasComponent implements OnInit, OnDestroy {
  constructor(
  ...
  gridPlantillasAsociadasConstancia = new KendoGrid<ConfiguracionPlantilla>();
  ...
  private initCombos() {
    ...
    this.comboPlantilla =
      this.pgeneralService.combosConfiguracionPlantilla.plantilaCertificadoConstancia.filter(
        (x) => x.idPlantillaBase == PlantillaBase.CONSTANCIA
      );
    this.comboEstados =
  ...
  private initSubscribeObservables() {
    ...
    let sub$ = this.pgeneralService.detalleProgramas$.subscribe((resp) => {
      if (resp != null) {
        ...
        this.gridPlantillasAsociadasConstancia.data = data;
      }
    });
    this.pgeneralService.getDatosModalPgeneral$.subscribe(() => {
      this.pgeneralService.dataConfiguracionPlantillaConstancia$.next(
        this.gridPlantillasAsociadasConstancia.data
      );
    });
  ...
  recuperarValoresForm() {
    ...
    let dataDetalle = this.gridCriterioConstancia.data;
    let item: ConfiguracionPlantilla = {
      ...
      idPlantillaBase: PlantillaBase.CONSTANCIA,
  ...
  insertarConstancia() {
    ...
    let dataPrincipal = this.gridPlantillasAsociadasConstancia.data;
    this.gridPlantillasAsociadasConstancia.data = [...dataPrincipal, item];
    this.modalRef.close();
  }
  /**
  ...
  private configurarGrid() {
    this.gridPlantillasAsociadasConstancia.removeEvent$.subscribe((resp) => {
      this._alertaService
        .swalFireOptions({
        ...
        .then((result) => {
          if (result.isConfirmed) {
            this.gridPlantillasAsociadasConstancia.data.splice(resp.index, 1);
            this.gridPlantillasAsociadasConstancia.data = [
              ...this.gridPlantillasAsociadasConstancia.data,
            ];
          }
