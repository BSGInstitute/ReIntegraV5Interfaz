...
@Component({
  selector: 'app-gestion-solicitudes-alumnos',
  templateUrl: './gestion-solicitudes-alumnos.component.html',
  styleUrls: ['./gestion-solicitudes-alumnos.component.scss'],
  encapsulation: ViewEncapsulation.None,
})

export class GestionSolicitudesAlumnosComponent implements OnInit {
  ...
  dataBusqueda: any[] = [
    ...
    { id: "DNI", nombre: "Nro de Documento" },
    { id: "NOMBRES", nombre: "Apellidos y Nombres" },
    { id: "CELULAR", nombre: "Número Teléfono" }
  ];

  EstadosSolicitud: any[] = [
    { id: 3, nombre: "Asignado a RS" },
    { id: 5, nombre: "En proceso de Solucion" },
    // { id: 6, nombre: "Observado por el  RS" },
    // { id: 7, nombre: "Resuelto por el RS" },
  ];
  ...
  buscarSolicitudesAlumno(idMatriculaCabecera :any){
    ...
    this.integraService
      .obtenerPorFiltro(constApiOperaciones.obtenerSolicitudesPorAlumnoGestion, filtro)
      .subscribe({
        next: (response: any) => {
          if (response != null) {
  ...
  filterAlumno(event: any) {
    //Autocomplete de Alumno
    event = event.trim();
    if (event.length >= 4) this.ObtenerAlumnoAutoComplete(event);
    else this.listaAlumno = [];
  }

  ObtenerAlumnoAutoComplete(alumno: string) {
    //matricula Autocomplete
    this.integraService
      .postJsonResponse(constApiOperaciones.ObtenerAlumnoPorValor, {valor: alumno})
      .subscribe({
        next: response => {
          this.listaAlumno = response.body;
        },
        error: error => {
          this.mostrarMensajeError("autocomplete - alumno");
        },
        complete: () => {}
      });
  }
  ...
  actualizarDatosSolicitudAlumno(){
    ...
    if((this.idNuevoEstado===undefined || this.idNuevoEstado==3) && this.inputDetalle!=""){
      this.alertaService.swalFire(
        'Error!',
    ...
    if((this.inputDetalle===""|| this.inputDetalle===undefined)&& (this.idNuevoEstado!=undefined || this.idNuevoEstado!=3)){
      this.alertaService.swalFire(
        'Error!',
