...
@Component({
  selector: 'app-reporte-certificado-instituto',
  templateUrl: './reporte-certificado-instituto.component.html',
  styleUrls: ['./reporte-certificado-instituto.component.scss']
})
export class ReporteCertificadoInstitutoComponent implements OnInit {
  ...
  datosEgresados = [
    {
      dni: '29391122',
      codigo: '10067276A1842',
      alumno: 'Oloartegui Lopez  Jeferson',
      pais: 'Perú',
      ciudad: 'Arequipa',
      telefono: 'e3 b0c 442 98f',
      correo: 'e24cada6@774b5802',
  ...
  cambiarCursoIrca(content: any, dataItem: any) {
    ...
    this.integraService
      .getJsonResponse(
        constApiOperaciones.AgendaInformacionActividadObtenerPEspecificoRelacionadoIrca +
          '/' +
          dataItem.idPEspecifico +
          '/' +
      ...
      .subscribe({
        next: (response: any) => {
          if (response.body.length == 0) {
            ...
            this.objCambiarCurso.idPEspecificoRecuperacion =
              dataItem.idPEspecifico;
            this.modalOpen = this.modalService.open(content, {
              size: 'md',
              backdrop: 'static',
  ...
  guardarRecuperarCurso() {
    if (
      this.objRecuperacion.idPEspecifico == 0 ||
      this.objRecuperacion.idPEspecifico == undefined
    ) {
      this.Toast.fire({
        icon: 'warning',
    ...
    this.objRecuperacion.idMatriculaCabecera =
      this.agendaService.rowActual.idMatriculaCabecera;
    this.objRecuperacion.idAlumno = this.agendaService.rowActual.idAlumno;
    this.objRecuperacion.idOportunidad =
    ...
    RecuperacionPEspecifico.IdPEspecificoRecuperacion =
      this.objRecuperacion.idPEspecificoRecuperacion; //Es el curso que se cambiara de estado a "recuperacion en otra modalidad"
  ...
  mostrarsesiones(dataItem: any) {
    ...
    this.integraService
      .getJsonResponse(
        constApiOperaciones.ProgramaEspecificoSesionObtenerSesionesAsociadosPEspecifico +
          '/' +
          dataItem +
          '/' +
          this.agendaService.rowActual.idMatriculaCabecera
  ...
  filterRecuperacionSesion(value: any) {
    this.inputPEspecificoRelacionadoDisponible =
      this.data_inputPEspecificoRelacionadoDisponible.filter(
