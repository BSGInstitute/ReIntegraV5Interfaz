...
import { CajaEgresoResumenCaja } from '@integra/models/caja-registro-egreso';
...
@Component({
  selector: 'app-tab-rec',
  templateUrl: './tab-rec.component.html',
  styleUrls: ['./tab-rec.component.scss']
})
export class TabRecComponent implements OnInit {
  ...
  listaSeleccion:any[]=[]
  listaREC:CajaEgresoResumenCaja[]=[]
  ...
  GenerarReporteREC(){//Genera el Reporte de NIC
      ...
      )
    {
      if(this.datosCaja && this.datosCaja.idCaja!=null)
      {
        ...
        this.integraService
        .obtenerPorPathParams(constApiFinanzas.ResumenCajaGenerarReporteREC,params)
        .subscribe({
          next: (response: HttpResponse<CajaEgresoResumenCaja[]>) => {
            console.log(response.body)
            this.listaREC=response.body
          },
          error: (error) => {
            this.loader=false
  ...
  CalcularTotal(){//Calcula el total efectivo de los seleccionados
    ...
    seleccion.forEach((e:any) => {
      this.total += this.listaREC.find(cp=>cp.idCajaEgresoAprobado===e).montoTotal
    });
  }
  ...
  limpiarSeleccion(){// Limpia la selección
   ...
   GenerarPDf()//Genera Un Zip Conteniendo los PDF
   {
     if(this.listaSeleccion.length>0)
     {
       this.loader=true
       this.integraService.
       obtenerPorFiltro(constApiFinanzas.ResumenCajaGenerarPdfREC,this.listaSeleccion)
       .subscribe({
         next: (response: HttpResponse<any>) => {
           ...
           this.listaSeleccion.forEach(e=>{
             let nic = this.listaREC.find(ln=>ln.idCajaEgresoAprobado===e).codigoEgresoCaja;
             nombrePdf.push(nic)
           })
           listaBytesPdf.forEach((bytesPdf:any)=>{
             ...
             zip.folder(this.datosCaja.codigoCaja+"-RECs").file(nombre, binary, { binary: true });
             i++
         });
         if (listaBytesPdf.length > 0) {
           zip.generateAsync({ type: "blob" })
           .then(function (content) {
             FileSaver.saveAs(content, "ResumenCaja-RECs");
           });
         }
         },
         error: (error) => {
